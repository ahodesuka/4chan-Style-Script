...
(function(){
            ...
            riceFile: function()
            {
                return this.each(function()
                {
                    ...
                    $(this).attr("riced", true)
                           .bind("change", function(){ $(this).nextSibling("span").text(this.value.substr(this.value.lastIndexOf("\\") + 1)); })
    ...
    $SS =
    {
        ...
        DOMLoaded: function(reload)
        {
            ...
            if (config["Custom Navigation Links"])
                ...
                postLoadCSS += "#navtop{bottom:2px;display:inline-block!important;height:18px;padding:3px 6px 6px;position:relative;width:500px;\
                                ...
                                #navtop::before{color:" + mascot.color.hex + ";content:'" + $(".logo span").text() + "';display:block;font-size:" + config["Small Font Size"] + "px}\
            ...
            if ((pages = $(".pages")).exists())
            {
                ...
                if (config["Pages Position"] == 1)
                {
                    ...
                    if (pages.exists())
                    {
                        ...
                        setTimeout(function(){ pages.attr("style", "left:" + ($(".pages td:last-child").get().offsetWidth - pages.get().offsetWidth) + "px"); }, 5);
                    }
                }
                else if (reload)
                    pages.attr("style", "");
            ...
            if ((!reload || !$(".riceFile").exists()) && config["Rice Inputs"] != 1)
            {
                ...
                if (!reload)
                    $(window).bind("load", initRice);
                else
                    initRice();
            }
            else if (reload && config["Rice Inputs"] == 1)
            {
                // unrice? is it even worth it?
                ...
                $(window).bind("load", function()
                {
                    ...
                    if ((qr = $("#qr")).exists())
                    {
                        var autohide = qr.children("#autohide"),
                            fixQRpos = function(){ if (!autohide.val()) autohide.parent().attr("style", "bottom:21px!important;-webkit-transition:bottom .1s;-moz-transition:bottom .1s;-o-transition:bottom .1s"); },
        ...
        {
            console.log(config["Rice Inputs"]);
            //if (e.target.nodeName == "TABLE" && config["Rice Inputs"] != 2)
            //   $("input[type=checkbox]", e.target).riceCheck();
            if (e.target.nodeName == "DIV" && config["Rice Inputs"] != 3)
