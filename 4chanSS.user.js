...
(function()
{
    ...
    $SS =
    {
        ...
        options:
        {
            ...
            showMascot: function(mIndex) // TODO: Fix base64 input element- why cant chrome hold all these chars- why is firefox and opera so slow
            {
                ...
                if (typeof mIndex === "number")
                    var bEdit = true,
                        mEdit = new $SS.Mascot(config["Mascots"][mIndex]);
                
                div = $("<div id=addMascot>").html("<label><span title='URL or base64'>Background:</span><textarea name=customIMG>" + (bEdit ? mEdit.img.img : "") + "</textarea></label>\
                        <label title='i.e. #FF6999'><span>Color (Hex.):</span><input type=text name=customColor value='" + (bEdit ? mEdit.color.hex : "") + "'></label>\
                        <label for=null><span>Alignment/Offset:</span>\
                        <select name=mPosition>\
                            <option" + (mEdit.position == "top" ? " selected" : "") + ">Top</option>\
                            <option" + (mEdit.position == "center" ? " selected" : "") + ">Center</option>\
                            <option" + (mEdit.position == "bottom" || !bEdit ? " selected" : "") + ">Bottom</option>\
                        </select>\
                        <input type=text name=mOffset value='" + (mEdit.offset || 0) + "px'></label>\
            ...
            {
                var overlay = $("#overlay2"), tIMG, div, cIMG, cColor, cPosition, cOffset, bSetPos = true;
                ...
                if (cPosition == "bottom" && (cOffset == 0 || cOffset == 22))
                    bSetPos = false;
                ...
                {
                    tIMG = new $SS.Image(cIMG);
                    var m = { img: cIMG, color: cColor, enabled: true };
                    ...
                    div = $("<div id=mascot" + (config["Mascots"].length - 1) + " class=selected style='background:" + tIMG.get() + "'><a title=Delete>X</a><a title=Edit>E</a>");
            ...
            getMascot: function()
            {
                var mascots = config["Mascots"], eMascot = [],
                    rnd, m;
        ...
        Theme: function(theme)
        {
            ...
            this.checkMark   = new $SS.Image(this.inputColor.isLight ? checkMarkDark : checkMarkLight, "no-repeat center");
        },
