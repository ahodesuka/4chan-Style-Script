    ...
    $SS =
    {
        ...
        options:
        {
            ...
            showMascot: function(mIndex) // TODO: Fix base64 input element- why cant chrome & opera hold all these chars- why is firefox and opera so slow
            {
                ...
                div = $("<div id=addMascot>").html("<label><span>Image:</span><input type=text name=customIMG value='" + (bEdit ? ($SS.validImageURL(mEdit.img) ? mEdit.img : "[Base 64 Encoded Image]") : "") + "'></label>\
                        ...
                        <span class=trbtn accept='image/GIF,image/JPEG,image/PNG'>Select Image File</span></div>\
                ...
                $("#selectImage>input[type=file]", div).bind("change", function()
                {
                    ...
                    reader.onload = (function(tImage)
                    {
                        return function(evt)
                        {
                            var b64 = $("<input type=hidden name=customIMGB64>").val($SS.cleanBase64(evt.target.result));
                            $this.after(b64);
                            $("input[name=customIMG]", div).val(fileName).attr("disabled", true);
                        }
                    })(image);
